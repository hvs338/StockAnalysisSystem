...
def auto_regressive_inference(tokenizer, model, x, x_stamp, y_stamp, max_context, pred_len, clip=5, T=1.0, top_k=0, top_p=0.99, sample_count=5, verbose=False):
    with torch.no_grad():
        ...
        device = x.device
        batch = x.size(0)
        x = x.unsqueeze(1).repeat(1, sample_count, 1, 1).reshape(batch * sample_count, x.size(1), x.size(2)).to(device)
        x_stamp = x_stamp.unsqueeze(1).repeat(1, sample_count, 1, 1).reshape(batch * sample_count, x_stamp.size(1), x_stamp.size(2)).to(device)
        y_stamp = y_stamp.unsqueeze(1).repeat(1, sample_count, 1, 1).reshape(batch * sample_count, y_stamp.size(1), y_stamp.size(2)).to(device)
...
class KronosPredictor:
    ...
    def predict(self, df, x_timestamp, y_timestamp, pred_len, T=1.0, top_k=0, top_p=0.9, sample_count=1, verbose=True):
        ...
        if len(y_timestamp) == 0:
            raise ValueError("y_timestamp is empty. Provide a DatetimeIndex with pred_len future timestamps.")
        if len(y_timestamp) < pred_len:
            raise ValueError(f"y_timestamp has {len(y_timestamp)} entries but pred_len={pred_len}. They must match.")
